var APP = {
    ...
    API_STATION_FIREBASE: 'https://stations-219cd-default-rtdb.firebaseio.com/stations.json',
    ...
    autocomplete: (stations) =>{
        ...
        container.innerHTML = ""
        //Affichage de la boîte d'autocomplétion
        container.classList.contains('none') ? container.classList.toggle('none') : null
        if (!stations.length) {
            container.innerHTML = `<div class="suggestion-item">Aucun résultat ne correspond à votre recherche</div>`
            return;
        }
